...
unordered_map<string, size_t> computeTupleDob(const set<vector<string>>& allClauses,
    ...
    unordered_map<string, bool> tupleinQueue;
    queue<string> q;
    for (const auto& t : allTuples) {
        if(allInputTuples.find(t) != allInputTuples.end()) {
            tupleDob[t] = 0;
            tupleinQueue[t] = true;
            q.push(t);
        } else {
            tupleDob[t] = numeric_limits<size_t>::max();
            tupleinQueue[t] = false;
        }
        //tupleDob[t] = allInputTuples.find(t) == allInputTuples.end() ? numeric_limits<size_t>::max() : 0;
    }

    while(!q.empty()) {
        string tuple = q.front();
        q.pop();
        tupleinQueue[tuple] = false;
        for(const auto &clause: tuple2AntecedentClauses[tuple]) {
            ...
            if (newDob < tupleDob[consequent]) {
                if(!tupleinQueue[consequent]) {
                    tupleinQueue[consequent] = true;
                    q.push(consequent);
                }
...
int main(int argc, char *argv[]) {
    ...
    /* add an argument for soft evidences */
    if (argc < 4) {
        cerr << __LOGSTR__ << "Insufficient arguments!" << endl
                           << "./prune-cons augment opTupleFileName softEvidenceFileName" << endl
    ...
    string softEvidenceFileName = argv[3];
    ...
    /* Load soft evidence as output to avoid non-reachable. */
    ifstream softEvidenceFile(softEvidenceFileName);
    while(softEvidenceFile >> tup) {
        allOutputTuples.insert(tup);
        double prob;
        softEvidenceFile >> prob;
    }
    clog << __LOGSTR__  << "Loaded " << allOutputTuples.size() << " output tuples and soft evidence tuples." << endl;
    ...
    const auto tupleDob = computeTupleDob(allClauses, allTuples, allInputTuples,tuple2AntecedentClauses);
    ...
    while (!unprocessedTuples.empty()) {
        ...
        if (tuple2ConsequentClauses.find(t) != tuple2ConsequentClauses.end()) {
        for (const auto& clause : tuple2ConsequentClauses.at(t)) {
