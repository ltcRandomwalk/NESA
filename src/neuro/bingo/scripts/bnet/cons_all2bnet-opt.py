...
softEvidenceFileName = sys.argv[2]
...
with open(softEvidenceFileName) as softEvidenceFile:
    line = softEvidenceFile.readline()
    while line:
        line = line.split(' ')
        assert len(line) == 2
        allSoftEvidences[line[0]] = line[1]
        line = softEvidenceFile.readline()
...
logging.info('Discovered {0} soft evidences.'.format(len(allSoftEvidences)))
...
for softEvidence in allSoftEvidences.keys():
    #assert softEvidence in allConsequents
    allNodes.append('_' + softEvidence)
    allNodes.append(softEvidence)
    allTuples.add('_' + softEvidence)
    allTuples.add(softEvidence)
...
print(len(allClauses) + len(nonSimpleConsequents)+2 * len(allSoftEvidences))
for node in allNodes:
    if node in allClauses:
        ...
        print(nodeStr)
    elif node in allConsequents:
        assert not node.startswith('_')
        t = node
        ...
        #assert node.startswith('_')
        #if node[1:] not in nodeIndex:
        #    continue
        parent = [ str(nodeIndex[node[1:]]) ]
        cosine = allSoftEvidences[node[1:]]
        nodeStr = '^ {0} {1} {2}'.format(len(parent), ' '.join(parent), str(cosine).strip())
        print(nodeStr)
    else:  # input nodes
        nodeStr = 'i 0'
        print(nodeStr)
