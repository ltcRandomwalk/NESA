...
with subprocess.Popen([wrapperExecutable, fgFileName], \
    ...
    time1 = time.time()
    statsFilename = testDir + '/naivebingo.txt'
    statsFile = open(statsFilename, 'w')
    runAlarmCarousel(tolerance, minIters, maxIters, histLength, statsFile, 0, 0)
    time2 = time.time()
