...
export SOFT_EVIDENCE_FILENAME=$4
...
if [ ! -f $SOFT_EVIDENCE_FILENAME ]; then
    >&2 echo "$SOFT_EVIDENCE_FILENAME not found!"
    exit 1
fi
...
./scripts/bnet/prune-cons/prune-cons $AUGMENT $OP_TUPLE_FILENAME $SOFT_EVIDENCE_FILENAME\
...
./scripts/bnet/cons_all2bnet-opt.py $PROGRAM_PATH/bnet/$AUGMENT_DIR/bnet-dict.out $SOFT_EVIDENCE_FILENAME narrowor \
    < $PROGRAM_PATH/bnet/$AUGMENT_DIR/named_cons_all.txt.pruned \
