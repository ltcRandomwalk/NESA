 ...
 */
@Chord(name = "soft-simple-ctxts-java",
       consumes = { "IM", "VH", "IK", "HK"},
       produces = { "C", "epsilonM", "kcfaSenM", "kobjSenM", "ctxtCpyM", "initCIC", "initCHC",
...
public class SimpleCtxtsAnalysis extends JavaAnalysis {
    ...
    public void run() {
        ...
        highestKobjValue = Integer.getInteger("chord.kobj.khighest", 10);
        assert (highestKobjValue > 0);
        highestKcfaValue = Integer.getInteger("chord.kcfa.khighest", 10);
        ...
        /*	String kvals = System.getProperty("chord.kvals", null);
			if (kvals != null) {
				...
				for (String elem : list) {
                    ...
                    case 'H': 
                    	kobjValue[idx] = val;
                    	highestKobjValue = highestKobjValue < val ? val : highestKobjValue;
                    	break;
                    case 'I': 
                    	kcfaValue[idx] = val;
                    	highestKcfaValue = highestKcfaValue < val ? val : highestKcfaValue;
                    	break;
		...
		*/	
        ...
        for (int hIdx = 1; hIdx < numA; hIdx++) {
            ...
            int kLim = (k+1) > highestKobjValue ? highestKobjValue : k+1;
            for(int j = 0; j <= kLim; j++){
            	for (Ctxt oldCtxt : ctxts) {
            		Quad[] oldElems = oldCtxt.getElems();
        ...
        for (int hIdx = 1; hIdx < numA; hIdx++) {
            ...
            for (Ctxt oldCtxt : ctxts) {
                Quad[] oldElems = oldCtxt.getElems();
                Quad[] newElems = combine(kLim, inst, oldElems);
                Ctxt newCtxt = domC.setCtxt(newElems);
