...
import chord.project.analyses.softrefine.FormatedConstraint;
import chord.project.analyses.softrefine.LookUpRule;
import chord.project.analyses.softrefine.MaxSatGenerator;
import chord.project.analyses.softrefine.ParamTupleConsHandler;
	...
	public Set<FormatedConstraint> getHardCons(int w, Set<Tuple> paramTSet, MaxSatGenerator gen) {
		...
		for(Map.Entry<Quad, List<Tuple>> entry: qToTMap.entrySet()){
			...
			for(int i = 0; i < l.size() - 1; i++){
				Tuple t1 = l.get(i);
				Tuple t2 = l.get(i+1);
				int i1 = gen.getIndex(t1);
				int i2 = gen.getIndex(t2);
				int cons[] = new int[2];
				cons[0] = 0-i1;
				cons[1] = i2;
