...
public class RelModCHA extends ProgramRel {
	public void fill() {
		...
		for (jq_Reference r : classes) {
			if (r instanceof jq_Array) {
				for (jq_InstanceMethod m : objClsInstanceMethods){
					...
					add(m,jq_NullType.NULL_TYPE,StubRewrite.maybeReplaceVirtCallDest(m, m, domM));
				}
				continue;
			}
					...
					for (jq_Reference s : classes) {
						...
						if (d.implementsInterface(c)) {
							...
							if (domM.contains(n)) {
								...
								add(m, jq_NullType.NULL_TYPE, StubRewrite.maybeReplaceVirtCallDest(m,n, domM));
							}
						}
					}
