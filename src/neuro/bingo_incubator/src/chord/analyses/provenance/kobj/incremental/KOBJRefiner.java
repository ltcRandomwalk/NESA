 ...
 * 
 */
@Chord(name = "kobj-refiner-incremaxsat")
public class KOBJRefiner extends JavaAnalysis {
	...
	private MaxSatGenerator createMaxSatGenerator(PTHandler ptHandler, int queryWeight) {
		...
		String isInc = System.getProperty("incSolver");
		if(isInc != null){
			if (gen_inc == null) {
				System.out.println("Create incremental solver ... ");
				gen_inc = new IncrementalMaxSatGenerator(configFiles, queryRelName, ptHandler, model, queryWeight);
				System.out.println("The incremental solver is created.");
				gen_inc.setQueryWeight(domH.size()+domI.size()+1);
			}
			return gen_inc;
		}
		else{
			MaxSatGenerator g = new MaxSatGenerator(configFiles, queryRelName, ptHandler, model, queryWeight);
			g.setQueryWeight(domH.size()+domI.size()+1);
			return g;
		}
