...
/*
 * This is an implementation of compositional RTA.
 * RTA analysis is performed on a set of training apps and the summaries learnt from them are stored.
 ...
 *                                          
 * chord.compoRTA.summaryDir[default ""]  string representing a directory where summaries are stored/read-from.
 * chord.compoRTA.flow [default ""]  string that specifies flow in the compositional analysis
 ...
 *                                   
 * chord.compoRTA.trainapps [default""] : comma-separated list of apps to get training data from.
 * chord.compoRTA.simulateSuperPerf [default false] simulate the super perfect scenario
 * chord.compoRTA.verify
 */

@Chord(name = "compoRTA", 
    consumes = {"T", "M", "I", "MI"}
)
public class CompoRTA extends JavaAnalysis {
	List<ITask> tasks;
	Timer timerSG;
	Timer timerSU;
	Timer timerSG1;
	...
	private RTAAnalysis analysis0;
	private RTAAnalysis analysis1;
	...
	public void run() {
		...
		String s = System.getProperty("chord.compoRTA.flow");
		...
		verify = Utils.buildBoolProperty("chord.compoRTA.verify", false);
		
		if (flowKind == FlowKind.BASELINE) {  // BASELINE RUN
			timerSG = new Timer("Baseline");
			timerSG.init();
			long initStart = System.nanoTime();
			analysis0 = new RTAAnalysis(true, false);  // baseline
			long initEnd = System.nanoTime();
			long initTime = initEnd - initStart;
			System.out.println("RTAAnalysis: INIT TIME: "+ initTime);
			System.out.println("RTAAnalysis: Readable INIT TIME: "+ Timer.getTimeStr(initTime/1000000));
			analysis0.run();
			timerSG.done();
			long timeSG = timerSG.getInclusiveTime();
			System.out.println("RTAAnalysis: BASELINE TIME: "+ Timer.getTimeStr(timeSG));
		}
		
		else if (flowKind == FlowKind.BASELINESUMMARY) {  // BASELINE RUN WITH FULL SUMMARY DUMP
			timerSG = new Timer("BaselineSummary");
			timerSG.init();
			analysis0 = new RTAAnalysis(true /* is baseline */, true /* dump summaries */);
			analysis0.run();
			timerSG.done();
			long timeSG = timerSG.getInclusiveTime();
			System.out.println("RTAAnalysis: BASELINE TIME WITH FULL SUMMARY DUMP: "+ Timer.getTimeStr(timeSG));
		}
			...
			timerSG.init();
			long initStart = System.nanoTime();
			analysis0 = new RTAAnalysis(false /* is not baseline */, true /* generate summary */);
			long initEnd = System.nanoTime();
			long initTime = initEnd - initStart;
			System.out.println("RTAAnalysis: INIT TIME: "+ initTime);
			System.out.println("RTAAnalysis: Readable INIT TIME: "+ Timer.getTimeStr(initTime/1000000));
			...
			timerSG.done();
			long timeSG = timerSG.getInclusiveTime();
			System.out.println("RTAAnalysis: SUMMARY GENERATION TIME: "+ Timer.getTimeStr(timeSG));
			
			if (verify) {
				timerSU = new Timer("Summary Use");
				timerSU.init();
				analysis1 = new RTAAnalysis(false /* is not baseline */, false /* consume summary */);
				analysis1.run();
				timerSU.done();
				long timeSU = timerSU.getInclusiveTime();
				System.out.println("RTAAnalysis: SUMMARY REUSE TIME: "+ Timer.getTimeStr(timeSU));
