...
public abstract class RHSAnalysis<PE extends IEdge, SE extends IEdge> extends JavaAnalysis {
	...
	****/
    ...
    private void processExit(BasicBlock bb, PE pe) {
        ...
        {
        	PE newMainEdge = propagateSEtoInit(m, seToAdd);
        	assert(newMainEdge != null);
        //	System.out.println("before remotion");
        //	System.out.println("");
        //	System.out.println(workList);
        	
        	Pair<Loc, PE> oldMainPair = workList.remove(0);
        	if (!oldMainPair.val0.i.getMethod().toString().contains("main"))
        	{
        		System.out.println(oldMainPair.val0.i.getMethod().toString());
        		assert(oldMainPair.val0.i.getMethod().toString().contains("main"));
        	}
       // 	System.out.println("removed "+ oldMainPair);
       // 	System.out.println("new Edge "+ newMainEdge);
        	Pair<Loc, PE> pair = new Pair<Loc, PE> (oldMainPair.val0, newMainEdge);
			workList.add(0, pair);
		//	System.out.println("after remotion");
		//	System.out.println("");
		//	System.out.println(workList);
        }
    }

    public abstract PE propagateSEtoInit(jq_Method m, SE seToAdd);
