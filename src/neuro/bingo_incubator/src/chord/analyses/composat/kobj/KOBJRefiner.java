 ...
 * -Dchord.provenance.preset=false: use the predefined abstraction and queries for current benchmark
 * @author xin
 * 
 */
@Chord(name = "kobj-refiner-composat")
public class KOBJRefiner extends JavaAnalysis {
	...
	// Map from benchmarks to the pair of queries and abstractions
	static Map<String,Pair<List<String>,List<String>>> predefineMap;

	static{
		predefineMap = new HashMap<String,Pair<List<String>,List<String>>>();
		
		// bloat
		{
			// we use an empty array to represent all queries are kept
			List<String> queries = new ArrayList<String>();
			List<String> abs = new ArrayList<String>();
			abs.add("650: NEW T305, EDU.purdue.cs.bloat.context.CachingBloatContext");
			abs.add("2: NEW T4, dacapo.parser.Config$Size");
			abs.add("518: NEW T346, EDU.purdue.cs.bloat.cfg.FlowGraph");
			abs.add("298: NEW T210, EDU.purdue.cs.bloat.trans.SSAPRE");
			abs.add("9: NEW T15, dacapo.bloat.BloatHarness");
			abs.add("46: NEW T51, EDU.purdue.cs.bloat.cfg.FlowGraph");	
			Pair<List<String>,List<String>> qbPair = new Pair<List<String>,List<String>>(queries,abs);
			predefineMap.put("bloat", qbPair);
		}
	}
	@Override
	public void run() {
		...
		usePredefined = Boolean.getBoolean("chord.provenance.preset");
		...
		System.out.println("chord.provenance.preset = "+usePredefined);
		...
		// fill in query and abs if we use predefined
		if(this.usePredefined){
			if(this.client != 0)
				throw new RuntimeException("Predefined queries and abs only support the polysite client!");
			String workDirTokens[] = Config.workDirName.split("/");
			String bench = workDirTokens[workDirTokens.length-1].toLowerCase().trim();
			Pair<List<String>,List<String>> qabsPair = predefineMap.get(bench);
			if(qabsPair != null){
				Set<Quad> clonedSites = new HashSet<Quad>();
				for(Object o : this.domH){
					Quad h = (Quad)o;
					if(h != null)
						if(qabsPair.val1.contains(h.toString())){
							clonedSites.add(h);
						}
				}
				if(qabsPair.val0.isEmpty()){
					for (Tuple t : unresolvedQs) {// put empty abstraction map for each query
						HashMap<Quad,Integer> abs = new HashMap<Quad,Integer>();
						for(Quad h : clonedSites)
							abs.put(h, 10);
						absMap.put(t, abs);
					}
				}else
				...
				throw new RuntimeException("No predefined query/abs pair for benchmakr");
		}
		else
			for (Tuple t : unresolvedQs) {// put empty abstraction map for each query
