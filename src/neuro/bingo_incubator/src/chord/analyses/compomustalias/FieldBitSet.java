...
public class FieldBitSet implements Set<jq_Field> {
	...
	public FieldBitSet(String s) {
		this();
		if (!s.equals("")) {
			String[] fldParts;
			fldParts = s.split("FLD_SEP");
			for (int i = 0; i < fldParts.length; i++) {
				int idx = Integer.parseInt(fldParts[i]);
				String fs = null;
				if (idx >= 0 && idx < fNdxToStrTrainApp.size())
					fs = fNdxToStrTrainApp.get(idx);
				if (fs != null && strToFNdx.containsKey(fs)) {
					int idx1 = strToFNdx.get(fs);
					bitSet.set(idx1);
				} else {
					badValue = true;
					break;
	...
	public String toParsableString() {
		...
		int startIndex = bitSet.nextSetBit(0);
		while(startIndex >= 0){
			//sv.append(domF.toUniqueString(startIndex));
			sv.append(startIndex);
			startIndex = bitSet.nextSetBit(startIndex+1);
			if (startIndex >= 0)
				sv.append("FLD_SEP");
		}
