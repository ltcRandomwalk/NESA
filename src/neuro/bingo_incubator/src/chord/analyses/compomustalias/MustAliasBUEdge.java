...
public class MustAliasBUEdge implements BUEdge<Edge, Edge>, Cloneable {
	...
	public MustAliasBUEdge(String s) {
		this();
		if (!s.equals("")) {
			String[] edgeParts;
			edgeParts = s.split("###");
			assert(edgeParts.length == 4);
			
			if (!edgeParts[0].equals("null")) {
				String[] vParts;
				vParts = edgeParts[0].split("VAR_SEP");
				for (int i = 0; i < vParts.length; i++) {
					Variable v = new Variable(vParts[i]);
					if (!v.badValue)
						killSet.add(v);
					else {
						badValue = true;
						//System.out.println("badValue in kill set");
						break;
					}
				}
			}
			if (!badValue && !edgeParts[1].equals("null")) {
				String[] vParts;
				vParts = edgeParts[1].split("VAR_SEP");
				for (int i = 0; i < vParts.length; i++) {
					String[] genParts;
					genParts = vParts[i].split("PAIR_SEP");
					Variable v0 = new Variable(genParts[0]);
					Variable v1 = new Variable(genParts[1]);
					if (!v0.badValue && !v1.badValue)
						genSet.add(new Pair<Variable, Variable>(v0, v1));
					else {
						badValue = true;
						//System.out.println("badValue in gen set");
						break;
					}
				}
			}
			if (!badValue && !edgeParts[2].equals("null")) {
				constraint = new AliasConstraint(edgeParts[2]);
				if (constraint.badValue) {
					badValue = true;
					//System.out.println("badValue in constraint");
				}
			}
			if (!badValue && !edgeParts[3].equals("null")) {
				int vIdx = Integer.parseInt(edgeParts[3]);
				String vs = null;
				if (vIdx >= 0 && vIdx < vNdxToStrTrainApp.size())
					vs = vNdxToStrTrainApp.get(vIdx);
				if (vs != null && strToVNdx.containsKey(vs)) {
					retRegister = domV.get(strToVNdx.get(vs));
				} else {
					badValue = true;
					//System.out.println("badValue in ret reg");
				}
			}
		}
	}
...
//		SortedSet<Constraint> tcCopy = new SortedArraySet<Constraint>(trackedCases);
		...
		for (int i = 0; i < args.length(); i++) {
			...
			if(!actualReg.getType().isReferenceType()) //ignore primitive type
				continue;
			Register formalReg = rf.get(i);
			curEdges = nextEdges;
			nextEdges = new ArraySet<BUEdge<Edge, Edge>>();
			if (domV.indexOf(formalReg) == -1) {
				System.out.println("Bad reg for Variable: " + domV.toUniqueString(formalReg) + "  " + i + "  " + tgtM);
			}
...
//			nextEdges = fitCases(nextEdges,tcCopy,bulimit);
		...
		for (BUEdge<Edge, Edge> ce : curEdges) {
		 ...
		 Register tgtRetReg = (Invoke.getDest(q) != null) ? Invoke.getDest(q)
		 .getRegister() : null;
		 if(tgtRetReg!=null&&this.retRegister!=null && this.retRegister.getType().isReferenceType()){
			 curEdges = nextEdges;
			 nextEdges = new ArraySet<BUEdge<Edge,Edge>>();
	...
	public String toParsableString() {
		...
		if (killSet.isEmpty())
			...
			boolean first = true;
			for (Variable v : killSet) {
				if (!first) sv.append("VAR_SEP");
				sv.append(v.toParsableString());
				first = false;
			}
		}
		sv.append("###");
		...
		if (genSet.isEmpty())
			...
			boolean first = true;
			for (Pair<Variable, Variable> p : genSet) {
				if (!first) sv.append("VAR_SEP");
				sv.append(p.val0.toParsableString());
				sv.append("PAIR_SEP");
				sv.append(p.val1.toParsableString());
				first = false;
			}
		}
		sv.append("###");
		
		sv.append(constraint.toParsableString());
		sv.append("###");
		
		if (retRegister == null)
			sv.append("null");
		else
			//sv.append(domV.toUniqueString(retRegister));
			sv.append(domV.indexOf(retRegister));
		sv.append("###");
