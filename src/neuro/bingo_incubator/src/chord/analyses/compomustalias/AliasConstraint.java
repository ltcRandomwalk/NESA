...
//	 over the allocation site
	...
	public AliasConstraint(String s) {
		this();
		if (!s.equals("")) {
			String[] consParts;
			consParts = s.split("%%%");
			assert(consParts.length == 5);
			
			if (consParts[0].equals("0"))
				isTrue = false;
			else if (consParts[0].equals("1"))
				isTrue = true;
			else
				badValue = true;
			
			if (consParts[1].equals("0"))
				isFalse = false;
			else if (consParts[1].equals("1"))
				isFalse = true;
			else
				badValue = true;
			
			if (!badValue && !consParts[2].equals("null")) {
				String[] vParts;
				vParts = consParts[2].split("VAR_SEP");
				for (int i = 0; i < vParts.length; i++) {
					...
					if (!v.badValue)
						...
						badValue = true;
						//System.out.println("badValue in inV set");
						break;
					}
				}
			}
			if (!badValue && !consParts[3].equals("null")) {
				String[] vParts;
				vParts = consParts[3].split("VAR_SEP");
				for (int i = 0; i < vParts.length; i++) {
					Variable v = new Variable(vParts[i]);
					if (!v.badValue)
						notInVSet.add(v);
						...
						badValue = true;
						//System.out.println("badValue in not inV set");
						break;
					}
				}
			}
			if (!badValue && !consParts[4].equals("null")) {
				notInFSet = new FieldBitSet(consParts[4]);
				if (notInFSet.badValue) {
					badValue = true;
					//System.out.println("badValue in notinF set");
				}
			}
		}
	}
	...
	public String toParsableString() {
		...
		if (notInFSet.isEmpty())
			sv.append("null");
		else
			sv.append(notInFSet.toParsableString());
