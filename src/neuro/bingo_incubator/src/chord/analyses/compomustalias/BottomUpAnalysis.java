...
import chord.project.analyses.tdbu.NoCaseMatchException;
import chord.project.analyses.tdbu.BUPESizeOverflowException;
import chord.project.analyses.tdbu.TDSummaryNotReadyException;
...
public abstract class BottomUpAnalysis<TDPE extends IEdge, TDSE extends IEdge, BUPE extends BUEdge<TDPE, TDSE>, BUSE extends BUEdge<TDPE, TDSE>> {
	 ...
	 */
	public Map<jq_Method, ArraySet<BUSE>> summEdges = new HashMap<jq_Method, ArraySet<BUSE>>();
	public Map<jq_Method, ArraySet<BUSE>> savedSummEdges = new HashMap<jq_Method, ArraySet<BUSE>>();
	...
	private long timeInBU = 0;
	private long timeInBUMethod = 0;

	private int BUTimes = 0;
	private int BUMatch = 0;
	private int BUUnmatch = 0;
	private int BUMethodTimes = 0;
	private int BUMethWLSize = 0;
	private int BUMethWLSizeApp = 0;
	private int BUMethWLSizeLib = 0;
	private int BUSummsComputed = 0;
	private int noCase = 0;
	private int caseExplode = 0;
	private int TDSENotReady = 0;

	private int numAppMethods = 0;
	private int numLibMethods = 0;
	private int numAppMethodsTimes = 0;
	private int numLibMethodsTimes = 0;
	
	public static boolean DEBUG = false;
	
	public SummaryHandler summH;
...
//		}
	...
	public int getBUMethodTimes() {
		return BUMethodTimes;
	}
	
	public int getBUMethWLSize() {
		return BUMethWLSize;
	}
	
	public int getBUMethWLSizeApp() {
		return BUMethWLSizeApp;
	}
	
	public int getBUMethWLSizeLib() {
		return BUMethWLSizeLib;
	}
	
	public int getBUSummsComputed() {
		return BUSummsComputed;
	}
	
	public int getNumAppMethods() {
		return numAppMethods;
	}
	
	public int getNumLibMethods() {
		return numLibMethods;
	}
	
	public int getNumAppMethodsTimes() {
		return numAppMethodsTimes;
	}
	
	public int getNumLibMethodsTimes() {
		return numLibMethodsTimes;
	}
	...
	public long getTimeInBU() {
		return timeInBU;
	}
	
	public long getTimeInBUMethod() {
		return timeInBUMethod;
	}
	...
	public Set<Pair<TDPE, Set<Pair<Loc, TDPE>>>> getBUInvkPathEdges(Quad q,
		...
		Set<Pair<TDPE, Set<Pair<Loc, TDPE>>>> ret = new ArraySet<Pair<TDPE, Set<Pair<Loc, TDPE>>>>();
		...
		for (BUSE buse : buses) {
			...
			if (ape != null) {
				...
				Set<Pair<Loc, TDPE>> tdInstStateSet = new ArraySet<Pair<Loc, TDPE>>();
	 ...
	 * @param tdSumms
	 */
	public void runBU(jq_Method root, Map<jq_Method, ArraySet<TDSE>> tdSumms) {
		...
		//if (i > 0) System.out.println("Start bu with " + root + " as the root");
		try {
			...
			for (; i >= 0; i--) {
				...
				for (jq_Method m : scci) {
					if (summH.mBelongsToLib(m))
						...
						numAppMethods++;
					ArraySet<BUSE> summs = summEdges.get(m);
					if (summs == null) {
						summs = new ArraySet<BUSE>();
...
//				}
	 ...
	 */
	protected boolean runBUonMethod(jq_Method m, ArraySet<TDSE> tdses)
		...
		try {
			...
			Map<Inst, ArraySet<BUPE>> pathEdges = new HashMap<Inst, ArraySet<BUPE>>();
			...
			Set<BUPE> bupesToLift = new ArraySet<BUPE>();
			...
			ArraySet<BUSE> newSumms = new ArraySet<BUSE>();
			...
			long endTime = System.nanoTime();
			timeInBUMethod += endTime - startTime;
		}
	}
	
	private ArraySet<BUSE> join(Set<BUSE> l, Set<BUSE> r){
		if(l.isEmpty())
			return new ArraySet<BUSE>(r);
		if(r.isEmpty())
			return new ArraySet<BUSE>(l);
		ArraySet<BUSE> ret = new ArraySet<BUSE>();
		for(BUSE se1 : l){
			for(BUSE se2 : r){
	...
	protected Set<BUPE> createSingleton(BUPE bupe) {
		Set<BUPE> ret = new ArraySet<BUPE>();
		ret.add(bupe);
		return ret;
	}
	...
	public boolean isMethodAnalyzed(jq_Method m) {
		//if (summH.consumeSummaries) summH.loadSEs(m);
	...
	protected void processInvk(Loc loc, BUPE bupe, MethodTuple mt)
		...
		if (targets.isEmpty()) {
			...
			for (jq_Method m2 : targets) {
				...
				Set<BUPE> bupesToAdd = new ArraySet<BUPE>();
				Set<Pair<Loc, BUEdge<TDPE, TDSE>>> instStatesToAdd = new ArraySet<Pair<Loc, BUEdge<TDPE, TDSE>>>();
				for (BUSE buse : seSet) {
					Set<BUPE> bupeSet1 = (ArraySet<BUPE>) buse.applyInvoke(q, bupe,
	...
	protected final Set<jq_Method> getTargets(Quad i) {
		...
		if (targets == null) {
			...
			targets = callGraph.getTargetsOrdered(i);
	...
	protected void addPathEdge(Loc loc, Set<BUPE> bupes, Loc preLoc,
		...
		for (BUPE bupe : bupeToAdd) {
			if (mt.trackedCases.contains(bupe.getConstraint())) {
				ArraySet<BUPE> bupes1 = mt.pathEdges.get(loc.i);
				if (bupes1 == null) {
					bupes1 = new ArraySet<BUPE>();
	...
	class MethodTuple {
		public jq_Method m;
		public ArraySet<TDSE> tdses;
		public Map<Inst, ArraySet<BUPE>> pathEdges;
