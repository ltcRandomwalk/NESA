package chord.analyses.incrsolver.sumgen;

import com.google.common.collect.Lists;

import java.io.ByteArrayOutputStream;
import java.io.PrintWriter;
import java.util.ArrayList;
import java.util.Iterator;

import gnu.trove.map.TObjectIntMap;
import gnu.trove.map.hash.TObjectIntHashMap;
import static com.google.common.base.Verify.verify;

/** A one-to-one correspondence between tuples and positive integers. */
public final class TupleIndex<T> implements Iterable<T> {
  public TupleIndex() {
    tupleDic = new TObjectIntHashMap<T>();
    tuplePool = Lists.newArrayList();
    tuplePool.add(null); // for 1-based indices
  }

  public int getIndex(T t) {
    int ret = tupleDic.get(t);
    if (ret <= 0) {
      verify(!frozen);
      ret = tuplePool.size();
      tupleDic.put(t, ret);
      tuplePool.add(t);
    }
    return ret;
  }

  public int getOldIndex(T t) {
    int r = tupleDic.get(t);
    verify(r > 0);
    return r;
  }

  public boolean hasIndex(T t) {
    return tupleDic.get(t) > 0;
  }

  // If you are very careful, you can also use indices above size() for
  // auxiliary variables, thus avoiding having to increase the tuplePool.
  public int getFreeIndex() {
    verify(!frozen);
    int idx = tuplePool.size();
    tuplePool.add(null);
    return idx;
  }

  public T getTuple(int i) {
    if (i >= tuplePool.size()) return null;
    return tuplePool.get(i);
  }
  public int size() { return tupleDic.size(); }

  @Override public Iterator<T> iterator() {
    Iterator<T> r = tuplePool.iterator();
    r.next();
    return r;
  }

  // DBG
  public void freeze() { frozen = true; }
  public void unfreeze() { frozen = false; }
  
  public void clear() {
	  tupleDic.clear();
	  tuplePool.clear();
	  tuplePool.add(null); // for 1-based indices
  }

  public void print(PrintWriter out) {
    for (int i = 1; i < tuplePool.size(); ++i) {
      T t = tuplePool.get(i);
      String s = t == null? "<null_tuple>" : t.toString();
      out.printf("%d %s%n", i, s);
    }
  }

  @Override public String toString() {
    ByteArrayOutputStream os = new ByteArrayOutputStream();
    PrintWriter pw = new PrintWriter(os);
    print(pw);
    pw.flush();
    pw.close();
    return os.toString();
  }

  private final TObjectIntMap<T> tupleDic;
  private final ArrayList<T> tuplePool;

  // DBG
  private boolean frozen = false;
}
