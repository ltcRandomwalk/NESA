...
# Note: all time and memory variables are stored using seconds and megabytes, respectively.
# TODO: differentiate machine from worker
...
def newHandle(prefix,n=6); prefix+'-'+(0...n).map { x = rand(36); x += x < 10 ? ?0 : ?a-10; x.chr }.join('') end
def encode(x); XMLRPC::Base64.encode(x) end
def decode(x); XMLRPC::Base64.decode(x) end

def renderTable(table, showHeaders) # First row is headers
...
class JobSpec
  ...
  def fromMap(map)
    ...
    if map['inPaths'] # Attach the most recent version of these
      @inVersionPaths += map['inPaths'].map { |path|
...
class Master
  ...
  def operateOnJobs(op, spec)
    ...
    when 'list'
      ...
      response['output'] = [['handle', 'status', 'time', 'mem', 'disk', 'outName', 'command']] + matchedJobs.map { |job|
        [job.spec.handle, job.status,
    ...
    if map['outPathContents']
      map['outPathContents'].each { |path,offsetContents|
        ...
        if offset == 'remote'
          ...
          out = open(path, File.exists?(path) ? 'r+' : 'w')
          out.seek(offset)
          out.write(contents)
          out.close
        end
...
class Worker
  ...
  def getOutPathContents(isLast)
    ...
    @job.spec.outModePaths.each { |mode,pathPattern|
      ...
      Dir[@basePath+pathPattern].each { |actualPath|
        ...
        #puts [isLast, newoffset, size].inspect
  ...
  def enforceLimits
    ...
    if spec.limitMem && state.mem && state.mem > spec.limitMem
      killCurrentJob("Current memory usage #{Format.space(state.mem)} exceeds limit #{Format.space(spec.limitMem)}")
    end
  ...
  def runJob(jobSpec)
    ...
    actualWorkingPath = @basePath+@job.spec.workingPath
    system('mkdir', '-p', actualWorkingPath)
    if @job.spec.outPath
      actualOutPath = @basePath+@job.spec.outPath
...
class Client
  ...
  def initialize(args)
    addCmd, directCmd, numTimes, specPath, priority, time, mem, disk, tags, workerHandle,
    ...
    availabilityZone, instanceType, imageId, showHeaders =
    ...
    if listSpec
      ...
      renderTable(response['output'], showHeaders).each { |line| puts line }
    end
