...
public class KCFAClientsAnalysis extends JavaAnalysis {
	...
	public void run() {
		...
		if((ctxtKind.equalsIgnoreCase("ci") && kVal == 0) || ctxtKind.equalsIgnoreCase("cs")){
		...
		Set<jq_Method> actual_reachableM_0cfa = new HashSet<jq_Method>();
		...
		ClassicProject.g().runTask("CM-dlog");
		...
		//	reachableM_0cfa.add(m0);
			...
			actual_reachableM_0cfa.add(m0);
			actual_reachableM_0cfa.add(m1);
			out1.println(m0 + " :: " + m1);
		}
		rel2.close();
		out1.close();
		...
		PrintWriter out4a = OutDirUtils.newPrintWriter("actual_reachableM_0cfa.txt");
		for (jq_Method m : actual_reachableM_0cfa) {
			out4a.println(m);
		}
		out4a.close();
		...
		ProgramRel rel10 = (ProgramRel) ClassicProject.g().getTrgt("allRacePairs");
		rel10.load();
		Iterable<Pair<Quad, Quad>> tuples10 = rel10.getAry2ValTuples();
		PrintWriter out10 = OutDirUtils.newPrintWriter("allRaceQueries_0cfa.txt");
		for (Pair<Quad, Quad> tuple : tuples10) {
			out10.println(tuple.val0 + " " + tuple.val1);
		}
		rel10.close();
		out10.close();
		
		ProgramRel rel11 = (ProgramRel) ClassicProject.g().getTrgt("racePairs_cs");
		rel11.load();
		Iterable<Pair<Quad, Quad>> tuples11 = rel11.getAry2ValTuples();
		PrintWriter out11 = OutDirUtils.newPrintWriter("failedRaceQueries_0cfa.txt");
		for (Pair<Quad, Quad> tuple : tuples11) {
			out11.println(tuple.val0 + " " + tuple.val1);
		}
		rel11.close();
		out11.close();
