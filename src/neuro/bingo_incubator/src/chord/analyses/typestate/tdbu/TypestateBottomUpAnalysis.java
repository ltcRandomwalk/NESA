...
public class TypestateBottomUpAnalysis extends
		BottomUpAnalysis<Edge, Edge, TypestateBUEdge, TypestateBUEdge> {
	private MyQuadVisitor qv;
	
	public TypestateBottomUpAnalysis(ICICG callGraph, int bULimit,int bUPELimit, Map<jq_Method, Set<jq_Method>> rmsMap, Set<jq_Method> noTDSEMs) {
		super(callGraph, bULimit, bUPELimit, rmsMap, noTDSEMs);
		qv = new MyQuadVisitor();
	}

	@Override
	protected Set<TypestateBUEdge> transfer(TypestateBUEdge inEdge, Quad q) {
		qv.identity(inEdge);
		q.accept(qv);
		return qv.output;
	}
	...
	@Override
	protected Pair<Loc, Set<TypestateBUEdge>> getInitialBUEdge(jq_Method m) {
		BasicBlock bb = m.getCFG().entry();
        Loc loc = new Loc(bb, -1);
        Set<TypestateBUEdge> retEdges = new HashSet<TypestateBUEdge>();
        Pair<Loc, Set<TypestateBUEdge>> ret = new Pair<Loc, Set<TypestateBUEdge>>(loc,retEdges);
        TypestateBUEdge initEdge = new TypestateBUEdge();
        retEdges.add(initEdge);
		return ret;
	}

	@Override
	protected Set<TypestateBUEdge> getDefaultSummaries(jq_Method m) {
		return new HashSet<TypestateBUEdge>();
	}
	...
	@Override
	protected TypestateBUEdge lift(TypestateBUEdge bupe, jq_Method m) {
		TypestateBUEdge ret = AliasUtilities.liftBUPE(bupe, m);
		return ret;
	}
	...
	public void printSummaries(PrintStream out){
		for(Map.Entry<jq_Method, Set<TypestateBUEdge>> entry: this.summEdges.entrySet()){
			...
			System.out.println();
			for(TypestateBUEdge edge: entry.getValue())
	...
	class MyQuadVisitor extends QuadVisitor.EmptyVisitor {
		TypestateBUEdge input;
		Set<TypestateBUEdge> output = new HashSet<TypestateBUEdge>();

		public void identity(TypestateBUEdge input) {
			this.input = input;
			output.clear();
			output.add(input);
		}
		...
		public void visitALoad(Quad obj) {
			...
			output.clear();
			Set<TypestateBUEdge> transSet = TypestateBUEdge.constructMoveNull(new Variable(ro.getRegister()));
			applyTrans(transSet);
		}
		...
		public void visitGetfield(Quad obj) {
			...
			Set<TypestateBUEdge> transSet = TypestateBUEdge.constructGetField(f, to);
			applyTrans(transSet);
		}
		...
		public void visitGetstatic(Quad obj) {
			...
			RegisterOperand tro = Getfield.getDest(obj);
			Variable to = new Variable(tro.getRegister());
			Set<TypestateBUEdge> transSet = TypestateBUEdge.constructMove(from, to);
			applyTrans(transSet);
		}
		...
		public void visitMove(Quad obj) {
			...
			if (ro instanceof RegisterOperand) {
				...
				Set<TypestateBUEdge> transSet = TypestateBUEdge.constructMove(
						from, to);
				applyTrans(transSet);
			}
			else{
				Set<TypestateBUEdge> transSet = TypestateBUEdge.constructMoveNull(to);
				applyTrans(transSet);
			}
		}
		...
		public void visitPutfield(Quad obj) {
			...
			if(src instanceof RegisterOperand){
				...
				Set<TypestateBUEdge> transSet = TypestateBUEdge.constructPutField(from, f);
				applyTrans(transSet);
			}else{
				Set<TypestateBUEdge> transSet = TypestateBUEdge.constructPutFieldNull(f);
				applyTrans(transSet);	
			}
		}
		...
		private void applyTrans(Set<TypestateBUEdge> transSet) {
			for (TypestateBUEdge trans : transSet) {
				TypestateBUEdge pos = (TypestateBUEdge) trans
						.apply(input, null);
				if (pos != null)
					output.add(pos);
			}
