...
//					return false;
	...
	public AliasConstraint statisfy(Set<Variable> minusSet,
			Set<Variable> unionSet, AliasConstraint cons) {
		if (this.isTrue())
			return cons;
...
//				+ ", notInFSet=" + notInFSet + "]";
	...
	public int size() {
		return inVSet.size()+notInVSet.size();
//				+notInFSet.size();
	}

	public AliasConstraint lift(Set<Register> args){
		Set<Variable> avs = new ArraySet<Variable>();
		for(Register r: args)
			avs.add(new Variable(r));
...
//		Set<Variable> newNotInV = Utilities.intersect(avs, notInVSet);
